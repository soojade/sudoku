...
(0, _jquery2.default)(function () {
  var grid = new _grid.Grid((0, _jquery2.default)('#box'));
  grid.build();

  // 选择等级
  (0, _jquery2.default)('#simple').on('click', function () {
    grid.build();
    console.log(grid);
  });
  (0, _jquery2.default)('#middle').on('click', function () {
    return grid.build(5);
  });
  (0, _jquery2.default)('#diffcult').on('click', function () {
    return grid.build(7);
  });

  // 生成弹出面板
  var popup = new _popup2.default((0, _jquery2.default)('#pop'));
  ...
  (0, _jquery2.default)('#clear').on('click', function () {
    return grid.clear();
  });
  (0, _jquery2.default)('#rebuild').on('click', function () {
    return grid.rebuild();
  });
});
...
var Grid = exports.Grid = function () {
  function Grid(container) {
    ...
    this.sudoku = new _sudoku2.default();
  }

  _createClass(Grid, [{
    key: 'build',
    value: function build(lv) {
      this.sudoku.makeSudoku(lv);
      var matrix = this.sudoku.sudoku;
      var cells = matrix.map(function (rowVal) {
        return rowVal.map(function (cellVal) {
          return (0, _jquery2.default)('<span>').addClass(cellVal ? 'fixed' : 'empty').text(cellVal);
        });
      }); // 创建9x9个span，并赋值
      var divArr = cells.map(function (span) {
        return (0, _jquery2.default)('<div>').append(span);
      });
    ...
    value: function check() {
      var data = this.sudoku.matrix; // 完整的数独
      var sudo = this.sudoku.sudoku; // 遮挡后的数独
      var arr = []; // 应该填入的正确答案
      for (var i = 0; i < 9; i++) {
        for (var j = 0; j < 9; j++) {
      ...
      var inputs = Array.from(this.container.find('span:not(.fixed)')); // 获取所有要输入的单元格元素
      var flags = []; // 设置标记，全部正确为true，提示成功
      inputs.forEach(function (item, index) {
        if (item.innerText != arr[index]) {
          // 比较填入内容和正确答案是否相同
          (0, _jquery2.default)(item).addClass('error');
          flags.push(false);
        } else {
          (0, _jquery2.default)(item).removeClass('error');
          flags.push(true);
        }
      });
      if (flags.every(function (i) {
        return i === true;
      })) {
        alert('恭喜你，成功通关！');
      }
    ...
    value: function reset() {
      this.container.find('span:not(.fixed)').removeClass('error sure nosure').addClass('empty').text(0);
    }
    ...
    key: 'bindPopup',
    value: function bindPopup(pop) {
      this.container.on('click', 'span', function (e) {
        var cell = (0, _jquery2.default)(e.target); // 转换成jquery对象
        if (!cell.hasClass('fixed')) {
          // 只有class是empty的单元格才能弹出面板
...
var Sudoku = function () {
  function Sudoku() {
    ...
    var generator = new _generator.Generator(); // 生成完整的数独
    generator.generate();
  ...
  _createClass(Sudoku, [{
    key: 'makeSudoku',
    value: function makeSudoku() {
      var level = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 4;

      this.sudoku = this.matrix.map(function (row) {
        return row.map(function (cell) {
          return Math.random() * 9 < level ? 0 : cell;
        });
      }); // 小于lv的填充0，如果填充''，当一行全是''时，不会绘制这一行
    }
  }]);
...
var Generator = exports.Generator = function () {
  ...
  _createClass(Generator, [{
    ...
    value: function generateMatrix() {
      this.matrix = _toolkit.MatrixToolkit.makeMatrix();
      this.orders = _toolkit.MatrixToolkit.makeMatrix().map(function (row) {
        return row.map(function (v, i) {
          return i;
        });
      }) // 获取矩阵索引
      .map(function (row) {
        return _toolkit.MatrixToolkit.shuffle(row);
      }); // 随机打乱索引
      for (var n = 1; n <= 9; n++) {
    ...
    value: function fillRow(n, rowIndex) {
      ...
      for (var i = 0; i < 9; i++) {
        ...
        if (!_toolkit.MatrixToolkit.checkFillable(this.matrix, n, rowIndex, colIndex)) {
...
exports.MatrixToolkit = MatrixToolkit;
exports.BoxToolkit = BoxToolkit;
...
var Popup = function () {
  ...
  _createClass(Popup, [{
    ...
    value: function popup(cell) {
      ...
      var width = this.panel.width(); // 弹出面板宽
      var height = this.panel.height(); // 弹出面板高
